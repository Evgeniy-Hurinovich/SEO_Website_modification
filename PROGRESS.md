...
| **F2** Еженедельный дашборд | ⏳ | Сборщик пишет `history.jsonl` + **`weekly_snapshot.csv`** (длинный контракт для Qlik). Сид lab 27.07–24.08. Витрина = Qlik; блокер: папка Folder Connection + ОК Миши. Чат B24 — запасной. |
...
5. **F2**: сборщик пишет CSV для Qlik (`--export-csv`). Папку Folder Connection ещё не дали.  
