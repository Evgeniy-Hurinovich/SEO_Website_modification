...
| **B4.2–B4.4** Отложить скрипты | ⏳ | У разработчика; ТЗ обновлено |
...
1. Разработчик: **B4.2–B4.3** — ТЗ `snippets/b4-defer-scripts-brief.md` (baseline 11.08 TBT **940 мс**; lab 20.08 **550 мс**).  
2. Хостер: поднять `opcache.max_accelerated_files` ≥100000; уточнить Redis host:port.  
3. Разработчик: MySQL `SET SESSION transaction_isolation='READ-COMMITTED'`; Redis в кеше Битрикс.  
4. **A4** — вывести `rel=canonical` в HTML.  
5. **F2** — еженедельный отчёт GSC.  
6. После B4 — повторный lab + PSI.
